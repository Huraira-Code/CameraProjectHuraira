...
import { Calendar as CalendarIcon, ArrowLeft, Loader2, Upload, Users, Info } from "lucide-react";
...
function EditPartnerEventPage() {
  ...
  React.useEffect(() => {
      async function fetchEventAndSettings() {
          setLoading(true);
          try {
            const [foundEvent, settingsSnap] = await Promise.all([
            ...
            if (settingsSnap.exists()) {
                const settingsData = settingsSnap.data() as PaymentSettings;
                const sortedTiers = settingsData.priceTiers?.sort((a,b) => a.guestCount - b.guestCount) || [];
                setPaymentSettings({ ...settingsData, priceTiers: sortedTiers });
            }
  ...
  const { priceTier, totalCalculatedPrice } = React.useMemo(() => {
    if (!paymentSettings || !event) {
        return { priceTier: null, totalCalculatedPrice: 0 };
    }
    
    const { priceTiers, basePrice } = paymentSettings;
    const guestCount = event.maxGuests || 0;

    if (!priceTiers || priceTiers.length === 0) {
        return { priceTier: null, totalCalculatedPrice: basePrice };
    }
    ...
    let total = suitableTier ? suitableTier.price : 0;

    // Add €10 for unlimited photos
                    ...
                    {paymentSettings && (
                        <div className="rounded-lg border p-3 shadow-sm space-y-4">
                           <div className="flex justify-between items-baseline">
                                <Label htmlFor="guest-slider" className="font-medium">Guest Limit & Price Indication</Label>
                            ...
                            />
                             <Alert>
                                ...
                                <AlertTitle>Retail Price Indication</AlertTitle>
                                <AlertDescription>
                                    A normal client would pay ~€{totalCalculatedPrice.toFixed(2)} for an event with {event.maxGuests} guests. This is for your reference only.
                                </AlertDescription>
                            </Alert>
                        </div>
                    )}
...
export default withAuth(EditPartnerEventPage, ['Partner', 'Admin']);
